...
def _card_yaml(**overrides) -> str:
    base = {
        ...
        "focus": ["Add hold_signal from lidar proximity",
...
class TestStrategyCardParsing:
    ...
    def test_accepts_raw_yaml_without_fence(self):
        ...
        card = parse_strategy_card(yaml.safe_dump({
...
class TestBuildStrategistPrompt:
    def test_contains_sections(self):
        p = build_strategist_prompt(
            ...
            top_candidates=[{"M1_success_rate": 0.01,
                             "M2_avg_return": -1.0,
        ...
        for marker in ("SEED BIAS", "CURRENT-RUN HISTORY",
    ...
    def test_mutation_log_summary_included(self):
        ...
        e = MutationLogEntry(
            ...
            run_id="r", task_id="t", seed=0, outer_iter=1,
            parent_version="v1", new_version="v1_mp_001",
            strategy_card={"target_domain": "reward",
                           "target_slot": "guidance_reward",
                           "focus": ["bounded shaping"]},
            slot_name="guidance_reward",
            slot_content_sha256="x", slot_content_excerpt="x",
            pre_mutation_peak_M1=0.0, pre_mutation_best_M6=0.0,
            post_mutation_peak_M1=0.0, post_mutation_best_M6=-0.05,
            delta_peak_M1=0.0, delta_M6=-0.05,
            verdict="regression",
        )
        p = build_strategist_prompt(
            history=[_rec()],
...
class TestStrategizeEndToEnd:
    def test_stub_llm_roundtrip(self):
        ...
        def stub(messages):
            ...
            return _card_yaml(target_domain="reward",
                              target_slot="guidance_reward",
                              focus=["Use tanh shaping"])
        card = strategize(
            history=[_rec()],
            mutation_log_entries=[],
            top_candidates=[{"M1_success_rate": 0.0,
                             "M2_avg_return": -2.0,
...
class TestClassifyVerdict:
    @pytest.mark.parametrize("pre,post,verdict", [
...
class TestEntryRoundtrip:
    def test_new_entry_and_append(self, tmp_path):
        e = new_entry(
            run_id="r1", task_id="t1", seed=0, outer_iter=1,
            parent_version="p", new_version="p_mp_001",
            strategy_card={"target_slot": "guidance_reward"},
            slot_name="guidance_reward",
            slot_content="a" * 200,
            pre_peak_M1=0.01, pre_M6=0.15,
        )
    ...
    def test_excerpt_truncated_when_long(self):
        e = new_entry(
            run_id="r", task_id="t", seed=0, outer_iter=0,
            parent_version="p", new_version="n",
            strategy_card={}, slot_name="s",
            slot_content="x" * 1000,
            pre_peak_M1=0.0, pre_M6=0.0,
            excerpt_chars=50,
        )
    ...
    def test_read_recent_filters_by_task(self, tmp_path):
        ...
        for i, task in enumerate(["t1", "t2", "t1", "t2", "t1"]):
            e = new_entry(
                run_id=f"r{i}", task_id=task, seed=0, outer_iter=i,
                parent_version="p", new_version=f"n{i}",
                strategy_card={}, slot_name="s",
                slot_content="c",
                pre_peak_M1=0.0, pre_M6=0.0,
            )
    ...
    def test_update_last_entry_resolves_post_fields(self, tmp_path):
        ...
        updated = update_last_entry_with_post(
            path, post_peak_M1=0.06, post_M6=0.20,
            fail_modes=["healthy"],
        )
...
class TestSummarizeForPrompt:
    ...
    def test_includes_version_and_verdict(self):
        e = MutationLogEntry(
            ts="2026-04-23T00:00:00Z", run_id="r", task_id="t",
            seed=0, outer_iter=0,
            parent_version="p", new_version="foo_mp_001",
            strategy_card={"target_domain": "observation",
                           "focus": ["hold signal"]},
            slot_name="guidance_observation",
            slot_content_sha256="x", slot_content_excerpt="",
            pre_mutation_peak_M1=0.0, pre_mutation_best_M6=0.0,
            post_mutation_peak_M1=0.08, post_mutation_best_M6=0.1,
            delta_peak_M1=0.08, delta_M6=0.1,
...
class TestEditorUsesStrategy:
    def test_editor_prompt_references_strategy_fields(self):
        ...
        p = build_editor_prompt(
            ...
            strategy_card=card,
            top_candidates=[{"M1_success_rate": 0.0, "M6_coverage_progress": 0.1,
                             "M2_avg_return": -1.0}],
            loader=None,  # unit-test mode (inline placeholders)
        )
        # All of these should appear in the editor prompt
        for marker in ("guidance_observation", "observation",
                       "hold_signal", "STRATEGY CARD",
                       "AVOID", "FOCUS", "FAIRNESS SLOT",
                       "DO NOT RESTATE"):
            assert marker in p, f"missing {marker!r}"
    ...
    def test_editor_prompt_includes_prior_slot_versions(self):
        ...
        prior = MutationLogEntry(
            ts="2026-04-23T00:00:00Z",
            run_id="r1", task_id="t", seed=0, outer_iter=1,
            parent_version="v_root", new_version="v_root_mp_001",
            strategy_card={}, slot_name="guidance_observation",
            slot_content_sha256="x" * 64,
            slot_content_excerpt="Use nearest target distance and simple proximity count.",
            pre_mutation_peak_M1=0.01, pre_mutation_best_M6=0.1,
            post_mutation_peak_M1=0.00, post_mutation_best_M6=0.08,
            delta_peak_M1=-0.01, delta_M6=-0.02,
            verdict="regression",
        )
        card = StrategyCard(
            target_domain="observation",
            ...
            confidence="medium", rationale="prior regressed",
        )
        p = build_editor_prompt(
            parent_version="v",
            strategy_card=card,
    ...
    def test_editor_prompt_omits_prior_block_when_empty(self):
        ...
        card = StrategyCard(
            target_domain="observation",
            target_slot="guidance_observation",
            focus=["x"], avoid=[], confidence="medium", rationale="",
        )
        p = build_editor_prompt(
            parent_version="v", strategy_card=card,
            top_candidates=[], loader=None,
...
class TestVerdictThresholdScaling:
    ...
    def test_update_last_entry_honors_scale(self, tmp_path):
        ...
        from src.lero.meta.mutation_log import (
            append_entry, new_entry, update_last_entry_with_post, read_recent,
        )
        path = tmp_path / "log.jsonl"
        e = new_entry(
            run_id="r", task_id="t", seed=0, outer_iter=0,
        ...
        updated = update_last_entry_with_post(
            path, post_peak_M1=0.05, post_M6=0.0, verdict_scale=10.0,
        )
...
class TestMultiPathReadRecent:
    ...
    def test_merge_two_logs_sorted_by_ts(self, tmp_path):
        import json, time
        from src.lero.meta.mutation_log import (
            MutationLogEntry, append_entry, read_recent,
        )
        ...
        for ts, ver, pth, seed in entries:
            e = MutationLogEntry(
                ts=ts, run_id="r", task_id="t", seed=seed, outer_iter=0,
                parent_version="p", new_version=ver,
                strategy_card={}, slot_name="s",
                slot_content_sha256="x", slot_content_excerpt="",
                pre_mutation_peak_M1=0.0, pre_mutation_best_M6=0.0,
            )
    ...
    def test_single_path_still_works(self, tmp_path):
        from src.lero.meta.mutation_log import (
            append_entry, new_entry, read_recent,
        )
        p = tmp_path / "log.jsonl"
        e = new_entry(
            run_id="r", task_id="t", seed=0, outer_iter=0,
            parent_version="p", new_version="only",
            strategy_card={}, slot_name="s",
            slot_content="c",
            pre_peak_M1=0.0, pre_M6=0.0,
        )
...
class TestReadPriorSlotVersions:
    ...
    def test_filters_by_slot_name(self, tmp_path):
        from src.lero.meta.mutation_log import (
            append_entry, new_entry, read_prior_slot_versions,
        )
        p = tmp_path / "log.jsonl"
        for i, slot in enumerate(
            ["guidance_observation", "guidance_reward",
             "guidance_observation", "guidance_shared",
             "guidance_observation"],
        ):
            e = new_entry(
                run_id="r", task_id="t", seed=0, outer_iter=i,
                parent_version="p", new_version=f"v{i}",
                strategy_card={}, slot_name=slot,
                slot_content=f"text-{i}",
                pre_peak_M1=0.0, pre_M6=0.0,
            )
            append_entry(p, e)
        obs = read_prior_slot_versions(
            p, slot_name="guidance_observation", task_id="t", n=10,
        )
    ...
    def test_limit_n(self, tmp_path):
        ...
        got = read_prior_slot_versions(
            p, slot_name="guidance_observation", task_id="t", n=3,
        )
        assert len(got) == 3
...
class TestPerSeedMetaTemperature:
    ...
    def test_three_way_cycle(self):
        from src.lero.meta.outer_loop import (
            SEED_META_TEMPERATURE, meta_temperature_for_seed,
        )

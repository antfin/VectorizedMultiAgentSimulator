...
the F7.5 Phase C preflight probe reads it to detect "your local source has
...
class CodeUploader:
    ...
    def upload(
        ...
        # Write the companion .code_hash blob — F7.5 Phase C preflight reads
...
def compute_local_code_hash(
    ...
    The F7.5 Phase C preflight probe uses this to compute the **local** hash
    ...
    kept = sorted(p for p in targets if not _is_excluded(p, repo_root, exclude_patterns))

...
def regenerate_videos(
    ...
    Reads cfg + checkpoint from ``<run_dir>``, rebuilds the experiment with
    the original seed (BEFORE → random-init policy), reloads the saved
    checkpoint (AFTER → trained policy), records both videos to
    ``<run_dir>/output/videos/``, and refreshes ``output/report.json`` so the
    video links resolve.

    Use this after pulling an OVH-trained run back to a local machine where
    Pyglet rendering works (the OVH job's video recording would have failed
    fail-soft per F6.6 Part 1).
    """
    config_path = run_dir / "input" / "config.json"
    if not config_path.is_file():
        typer.echo(f"✗ no input/config.json under {run_dir}", err=True)

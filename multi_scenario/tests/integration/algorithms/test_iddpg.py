"""F3.3 integration tests: IddpgAdapter — Protocol + off-policy smoke training."""
...
def test_iddpg_implements_algorithm_protocol():
    """IddpgAdapter satisfies the Algorithm port."""
    assert isinstance(IddpgAdapter(), Algorithm)


def _smoke_config(tmp_path: Path) -> ExperimentConfig:
    return ExperimentConfig.model_validate(
        {
            "experiment": {"id": "iddpg_smoke", "seed": 0},
            "scenario": {
                "type": "discovery",
                "params": {
                    "n_agents": 2,

"""F3.4 integration tests: IsacAdapter — Protocol + off-policy SAC smoke training."""
...
def test_isac_implements_algorithm_protocol():
    """IsacAdapter satisfies the Algorithm port."""
    assert isinstance(IsacAdapter(), Algorithm)


def _smoke_config(tmp_path: Path) -> ExperimentConfig:
    return ExperimentConfig.model_validate(
        {
            "experiment": {"id": "isac_smoke", "seed": 0},
            "scenario": {
                "type": "discovery",
                "params": {
                    "n_agents": 2,

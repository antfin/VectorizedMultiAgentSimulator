"""F3.2 integration tests: MaddpgAdapter — Protocol + off-policy smoke training."""
...
def test_maddpg_implements_algorithm_protocol():
    """MaddpgAdapter satisfies the Algorithm port."""
    assert isinstance(MaddpgAdapter(), Algorithm)


def _smoke_config(tmp_path: Path) -> ExperimentConfig:
    return ExperimentConfig.model_validate(
        {
            "experiment": {"id": "maddpg_smoke", "seed": 0},
            "scenario": {
                "type": "discovery",
                "params": {
                    "n_agents": 2,
...
def test_smoke_training_2_iter(tmp_path: Path):
    ...
    First off-policy algorithm in the framework — exercises the off_policy_*
    knob wiring in BenchmarlBaseAdapter._experiment_config.
    """
    adapter = MaddpgAdapter()
    ...
    # Discovery-specific: targets_covered cumsum tensor.

...
def _fake_rollout_td(
    num_envs: int = 2, t_steps: int = 3, n_agents: int = 2, action_dim: int = 2
):
    ...
    e_idx = (
        torch.arange(num_envs).view(num_envs, 1, 1).expand(num_envs, t_steps, n_agents)
    )
    t_idx = (
        torch.arange(t_steps).view(1, t_steps, 1).expand(num_envs, t_steps, n_agents)
    )
    a_idx = (
        torch.arange(n_agents).view(1, 1, n_agents).expand(num_envs, t_steps, n_agents)
    )
    reward = (e_idx * 100 + t_idx * 10 + a_idx).float().unsqueeze(-1)  # [E,T,A,1]
    action = (
        e_idx.float()
        .unsqueeze(-1)
        .expand(num_envs, t_steps, n_agents, action_dim)
        .clone()
    )
...
def test_save_eval_steps_long_reward_values_correct(tmp_path: Path) -> None:
    ...
    row = df[
        (df["env_idx"] == 1) & (df["step"] == 2) & (df["agent"] == "agents:agent_0")
    ]
    assert len(row) == 1
...
def test_save_eval_steps_long_creates_output_dir(tmp_path: Path) -> None:
    ...
    LocalStorageAdapter().save_eval_steps_long(
        tmp_path, td, group_map={"agents": ["agent_0"]}
    )
